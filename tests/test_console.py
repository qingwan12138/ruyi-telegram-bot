...
from telegram_bot.console_routes import sanitize_endpoint_for_display
...
def test_sanitize_endpoint_removes_credentials_query_and_fragment() -> None:
    endpoint = "https://user:pass@example.com:8443/callback/path?token=SECRET#debug"

    assert sanitize_endpoint_for_display(endpoint) == (
        "https://example.com:8443/callback/path"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("http://example.com:80/callback", "http://example.com/callback"),
        ("http://example.com:8080/callback", "http://example.com:8080/callback"),
        ("https://[2001:db8::1]:8443/callback", "https://[2001:db8::1]:8443/callback"),
        ("https://example.com:not-a-port/callback", "https://example.com/callback"),
    ],
)
...
def test_console_status_exposes_only_safe_configuration_summary() -> None:
    settings = Settings(
        ...
        callback_forward_url=(
            "https://fallback-user:fallback-pass@fallback.local/callback"
            "?token=FALLBACKSECRET#debug"
        ),
        callback_targets={
            "test-workflow": (
                "https://target-user:target-pass@test.local/callback"
                "?token=TARGETSECRET#fragment"
            ),
            "package-pr-workflow": "http://package.local:8080/result",
        },
    )
            ...
            "targets": [
                {
                    ...
                    "endpoint_display": "http://package.local:8080/result",
                },
                {
                    "name": "test-workflow",
                    "endpoint_display": "https://test.local/callback",
                },
            ],
            "legacy_fallback_configured": True,
            "legacy_endpoint_display": "https://fallback.local/callback",
        },
